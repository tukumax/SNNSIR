from utils import load_yaml_config, save_yaml_config, PerceptualLoss, \
    generate_timestamp, K15_dataset, StereoWaterdrop_dataset,StereoSR_dataset, torchPSNR, torchSSIM, mkdir, get_logger
...
def parse_args():
    ...
    parser.add_argument('--opt', type=str, default='./configs/unet5_wwLifMul_1CAM_snn_sr_32to160_vth02_SRBB_SSCM_SSCA_SSRB_1e3_4_c32.yml' , help='Path to option YAML file.')
...
def train(opt, yaml_path):
    ...
    if dataset_opt['kind'] == 'k15':
        ...
        val_loader = DataLoader(dataset=dataset_val, num_workers=dataset_opt['val']['num_worker_per_gpu'],
                                batch_size=dataset_opt['val']['batch_size'],
                                shuffle=False, drop_last=False)
    elif dataset_opt['kind'] == 'StereoSR':
        # root, crop_size = [30, 90], scale = 2, mode = 'test'
        dataset_train = StereoSR_dataset(dataset_opt['train']['dataroot'], dataset_opt['train']['patch_size'],
                                                opt['network']['sr_scale'],'train')
        train_loader = DataLoader(dataset=dataset_train, num_workers=dataset_opt['train']['num_worker_per_gpu'],
                                  batch_size=dataset_opt['train']['batch_size'],
                                  shuffle=dataset_opt['train']['use_shuffle'],
                                  drop_last=False)
        dataset_val = StereoSR_dataset(dataset_opt['val']['dataroot'], dataset_opt['val']['patch_size'], opt['network']['sr_scale'],'valid')
        val_loader = DataLoader(dataset=dataset_val, num_workers=dataset_opt['val']['num_worker_per_gpu'],
                                batch_size=dataset_opt['val']['batch_size'],
                                shuffle=False, drop_last=False)
    ...
    for epoch in range(start_epoch, train_opt['epochs'] + 1):
        ...
        with torch.amp.autocast('cuda'):
            for i, data in enumerate(train_loader, 0):
                ...
                for res_l, tar_l, res_r, tar_r in zip(restored_l, gt_l, restored_r, gt_r):
                    train_psnr_rgb.append((torchPSNR(res_l, tar_l).item() + torchPSNR(res_r, tar_r).item()) / 2)
                train_ssim_rgb.append((torchSSIM(restored_l, gt_l).item() + torchSSIM(restored_r, gt_r).item()) / 2)
                writer.add_scalar('loss/iter_loss', loss.item(), iter)

        psnr_train = sum(train_psnr_rgb) / len(train_psnr_rgb)
        ssim_train = sum(train_ssim_rgb) / len(train_ssim_rgb)
        ...
        if epoch % dataset_opt['val']['val_epochs'] == 0:
            ...
            for i, data in enumerate(val_loader, 0):
                ...
                for res_l, tar_l, res_r, tar_r in zip(restored_l, gt_l, restored_r, gt_r):
                    val_psnr_rgb.append((torchPSNR(res_l, tar_l).item() + torchPSNR(res_r, tar_r).item()) / 2)
            psnr_val = sum(val_psnr_rgb) / len(val_psnr_rgb)
